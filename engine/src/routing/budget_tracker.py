...
def _budget_state_path() -> str:
    ...
    data_paths.shared_dir()  # ensure directory exists
...
class BudgetTracker:
    ...
    monthly_soft_cap : float, optional
        Soft warning threshold (defaults to 80% of hard cap).
    ...
    def _load_or_create(
        ...
        if os.path.isfile(self._state_path):
            try:
                ...
                # Update caps from config (may have changed)
                ...
                log.warning("Corrupt budget state file, creating fresh.")
    ...
    def _check_month_rollover(self):
        ...
        if self.state.month_key != current_month:
            log.info("Month rollover %s → %s — resetting monthly budget.",
    ...
    def update_caps(
        ...
        monthly_soft_cap: Optional[float] = None,
        per_session_cap: Optional[float] = None,
        per_tick_cap: Optional[float] = None,
    ):
        """Update spending caps at runtime (e.g. from UI/AGI config)."""
        if monthly_hard_cap is not None:
            self.state.monthly_hard_cap = monthly_hard_cap
        if monthly_soft_cap is not None:
            self.state.monthly_soft_cap = monthly_soft_cap
        if per_session_cap is not None:
            self.state.per_session_cap = per_session_cap
        if per_tick_cap is not None:
            self.state.per_tick_cap = per_tick_cap
        self._save()
